...
public class CompanyPane extends GridPane {
	...
	public CompanyPane() {
		...
		Label lblCstm = new Label("Customers:");
		this.add(lblCstm,1,4);
		GridPane.setValignment(lblCstm, VPos.BASELINE);
		lblCstm.setPadding(new Insets(4,0,4,0));
		...
		txaCstm = new TextArea();
		this.add(txaCstm,2,4);
		txaCstm.setPrefWidth(200);
		txaCstm.setPrefHeight(100);
		txaCstm.setEditable(false);

		HBox hbxButtons = new HBox(40);
		this.add(hbxButtons, 0, 4, 1, 1);
		...
		btnCustomer.setOnAction(event -> this.createCustomer());
	...
	private void createCustomer(){
		CustomerWindow dias = new CustomerWindow("Create customer");
		dias.showAndWait();
		updateControls();
	...
	public void updateControls() {
		...
		if (company != null) {
			...
			for(Customer cstm : company.getCustomer()){
				sb2.append(cstm + "\n");
			}
			txaEmps.setText(sb.toString());
			txaCstm.setText(sb2.toString());
